...
export default function BloomingView({
  ...
  const pingSnapshotImage = async () => {
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        const res = await fetch(snapshotImageUrl, {
          method: "HEAD",
          cache: "no-store",
        });
      ...
      <div className="relative z-10 px-4 pt-8 pb-8 max-w-md mx-auto">
            ...
            {showReveal && (
              ...
              >
                <Image
                  ...
                  onError={(e) => {
                    console.log(
                      `🌸 [BloomingView IMAGE] Error loading image (attempt ${
                        imageErrorCount + 1
                      }), trying fallback`
                    );
                    ...
                    if (newErrorCount > 3) {
                      console.log(
                        "🌸 [BloomingView IMAGE] Max retries reached, using final fallback"
                      );
                      setCurrentImageSrc(
                        "https://d17wy07434ngk.cloudfront.net/seed1/seed.png"
                      );
                    ...
                    if (newErrorCount <= fallbackImages.length) {
                      ...
                      console.log(
                        `🌸 [BloomingView IMAGE] Trying fallback: ${fallbackSrc}`
                      );
        ...
        <div className="text-center lg:text-[22px] md:text-[21px] text-[18px] scale-[0.8] lg:scale-[0.7] md:scale-[0.7] leading-4 lg:leading-8 md:leading-8 text-black mb-4">
          <div className="">
            ...
            <span className="text-wrap"> and here is how the </span>
          </div>
          <p className="text-nowrap -ml-10 lg:-ml-8 md:-ml-7">
            {`plant's`} morphology carries this as memory
          </p>
        </div>
