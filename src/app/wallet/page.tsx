...
import {
  getCodeForIndex,
  codeToSeedEmblemPath,
} from "@/lib/data/beneficiaryIndexMap";
...
const getInitialCachedSnapshots = (walletAddress: string | null): any[] => {
  console.log(
    "🚀 [WALLET INIT] getInitialCachedSnapshots called with walletAddress:",
    walletAddress
  );
  ...
  try {
    ...
    if (!cached) {
      console.log(
        "[WALLET INIT] No cached data found in localStorage for key:",
        cacheKey
      );
      return [];
    }
    ...
    if (isExpired) {
      console.log(
        "⏰ [WALLET INIT] Cache expired, clearing and returning empty array"
      );
    ...
    console.log(
      "✅ [WALLET INIT] Loaded cached snapshots from localStorage:",
      cacheData.snapshots?.length || 0,
      "snapshots"
    );
...
export default function WalletPage() {
  ...
  const [beneficiaryLinks, setBeneficiaryLinks] = useState<Map<number, string>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    console.log(
      "🔍 [WALLET] Current state - allSnapshots.length:",
      allSnapshots.length,
      "tendedEcosystems.length:",
      tendedEcosystems.length
    );
    ...
    if (walletAddress && allSnapshots.length === 0) {
      ...
      if (cachedSnapshots.length > 0) {
        console.log(
          "⚡ [WALLET] Wallet address now available, loading cache immediately"
        );
  ...
  const cacheSnapshots = (snapshots: any[], address: string) => {
    ...
    try {
      ...
      console.warn(
        "⚠️ [WALLET] Failed to cache snapshots to localStorage:",
        error
      );
    }
  };
  ...
  const loadMoreSnapshots = async () => {
    ...
    try {
      const snapshotsUrl = `${API_CONFIG.baseUrl}${API_ENDPOINTS.userSnapshots(
        walletAddress
      )}`;
        ...
        if (data.success && data.snapshots) {
          ...
          const enrichedSnapshots = await Promise.all(
            data.snapshots.map(async (snapshot: any) => {
              try {
                const seedData = await fetchSeedById(
                  snapshot.seedId.toString()
                );
                const beneficiary = await fetchBeneficiaryByIndex(
                  snapshot.beneficiaryIndex
                );

                let ecosystemCompost = "0.00 ETH";
                try {
                  const seedStats = await fetchSeedStats(
                    snapshot.seedId.toString()
                  );
                  if (
                    seedStats?.beneficiaries &&
                    Array.isArray(seedStats.beneficiaries)
                  ) {
                    ...
                    if (matchingBeneficiary?.totalValue) {
                      const rawValue = parseFloat(
                        matchingBeneficiary.totalValue
                      );
                  ...
                  console.warn(
                    `Failed to fetch seed stats for snapshot ${snapshot.id}:`,
                    statsError
                  );
                }
                ...
                return {
                  ...
                  beneficiaryName:
                    beneficiary?.name ||
                    `Beneficiary #${snapshot.beneficiaryIndex}`,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      console.log(
        "🔗 [WALLET] Starting to load wallet data for address:",
        walletAddress
      );
      ...
      try {
        ...
        const seedsUrl = `${API_CONFIG.baseUrl}${API_ENDPOINTS.userSeeds(
          walletAddress
        )}`;
        ...
        const seedsResponse = await fetch(seedsUrl, {
          headers: seedsPrevEtag
            ? { "If-None-Match": seedsPrevEtag }
            : undefined,
        });
        console.log(
          "📊 [WALLET] Seeds API Response status:",
          seedsResponse.status
        );

        if (seedsResponse.status === 304) {
          console.log(
            "✅ [WALLET] Seeds not modified (ETag). Skipping state update."
          );
          ...
          console.error(
            "[WALLET] Seeds API request failed with status:",
            seedsResponse.status
          );
          if (!cancelled) {
            setStewardSeeds([]);
        ...
        if (hasCachedData && !shouldRefresh) {
          console.log(
            "✅ [WALLET] Using cached data, skipping API fetch (",
            cachedSnapshots.length,
            "snapshots)"
          );
          ...
          console.log(
            "🔄 [WALLET] Fetching fresh snapshots data (hasCachedData:",
            hasCachedData,
            "shouldRefresh:",
            shouldRefresh,
            ")"
          ...
          const snapshotsUrl = `${
            API_CONFIG.baseUrl
          }${API_ENDPOINTS.userSnapshots(walletAddress)}`;
          ...
          const shouldThrottle =
            allSnapshots.length > 0 && lastMs && nowMs - lastMs < 20_000;

          let snapshotsResponse: Response | null = null;
          if (!shouldThrottle) {
            snapshotsResponse = await fetch(snapshotsUrl, {
              headers: prevSnapsEtag
                ? { "If-None-Match": prevSnapsEtag }
                : undefined,
            });
          } else {
            console.log(
              "⏭️ [WALLET] Skipping snapshots refetch (throttled <20s)."
            );
          }

          if (snapshotsResponse) {
            console.log(
              "📊 [WALLET] API Response status:",
          ...
          if (
            ...
            snapshotsResponse.status !== 304
          ) {
            throw new Error(
              `API request failed with status ${snapshotsResponse.status}`
            );
          }
          ...
          if (snapshotsResponse && snapshotsResponse.status !== 304) {
            ...
            if (snapshotsData.success && snapshotsData.snapshots.length > 0) {
              console.log(
                "✅ [WALLET] Loaded snapshots:",
                snapshotsData.snapshots
              );
              console.log(
                "🔍 [WALLET] First snapshot structure:",
              ...
              const enrichedSnapshots = await Promise.all(
                snapshotsData.snapshots.map(async (snapshot: any) => {
                  try {
                    ...
                    console.log(
                      `🔍 [WALLET] Beneficiary data for index ${snapshot.beneficiaryIndex}:`,
                      beneficiary
                    );
                    ...
                    try {
                      console.log(
                        `🔍 [WALLET] Fetching seed stats for seedId: ${
                          snapshot.seedId
                        } (type: ${typeof snapshot.seedId}), beneficiaryCode: ${
                          beneficiary?.code
                        }`
                      );
                      const seedStats = await fetchSeedStats(
                      ...
                      console.log(
                        `🔍 [WALLET] Seed stats response for seedId ${snapshot.seedId}:`,
                        seedStats
                      );
                      ...
                      if (
                        ...
                        console.log(
                          `🔍 [WALLET] Available beneficiaries in stats:`,
                          seedStats.beneficiaries.map((b: any) => ({
                            code: b.code,
                        ...
                        const matchingBeneficiary =
                          seedStats.beneficiaries.find(
                            (b: any) => b.code === beneficiary?.code
                          );

                        console.log(
                          `🔍 [WALLET] Looking for beneficiary code: ${beneficiary?.code}`
                        );
                        console.log(
                          `🔍 [WALLET] Matching beneficiary found:`,
                          matchingBeneficiary
                        );
                        ...
                        if (matchingBeneficiary?.totalValue) {
                          ...
                          ecosystemCompost = `${rawValue.toFixed(6)} ETH`;
                          console.log(
                            `🔍 [WALLET] Raw totalValue: ${matchingBeneficiary.totalValue}, parsed: ${rawValue}, formatted: ${ecosystemCompost}`
                          );
                        } else {
                          console.log(
                            `🔍 [WALLET] No matching beneficiary or totalValue found`
                          );
                        }
                      } else {
                        console.log(
                          `🔍 [WALLET] No beneficiaries array in seed stats`
                        );
                      }
                    } catch (statsError) {
                      console.warn(
                    ...
                    if (beneficiary?.projectData?.readMoreLink) {
                      setBeneficiaryLinks((prev) =>
                        new Map(prev).set(
                          snapshot.beneficiaryIndex,
                          beneficiary.projectData!.readMoreLink!
                        )
                      );
                    }
                    ...
                    console.log(
                      `🔍 [WALLET] Enriched snapshot ${snapshot.id}:`,
                      {
                    ...
                    console.error(
                      `Failed to enrich snapshot ${snapshot.id}:`,
                      e
                    );
                    ...
                    console.log(
                      `🔍 [WALLET] Fallback snapshot ${snapshot.id}:`,
                      {
                        originalImageUrl: snapshot.imageUrl,
              ...
              if (!cancelled) {
                ...
                const endIndex = currentPage * snapshotsPerPage;
                setTendedEcosystems(
                  reversedSnapshots.slice(startIndex, endIndex)
                );
                setHasMoreSnapshots(
                  reversedSnapshots.length > snapshotsPerPage
                );

                // Cache the data
                cacheSnapshots(reversedSnapshots, walletAddress);
                console.log(
                  "✅ [WALLET] Enriched snapshots (newest first):",
                  reversedSnapshots
                );
              }
              ...
              if (newSnapsEtag)
                setCookie(snapsEtagCookieKey, newSnapsEtag, 60 * 10); // 10 minutes
              setCookie(snapsLastCookieKey, String(Date.now()), 60 * 10);
            } else {
              console.log(
                "[WALLET] No snapshots found or API failed:",
                snapshotsData
              );
              if (!cancelled) {
                setAllSnapshots([]);
            ...
            // 304 or throttled
            console.log(
              "✅ [WALLET] Using existing snapshots (304/throttled)."
            );
          }
    ...
    <div className="min-h-screen w-full max-w-md mx-auto overflow-x-hidden">
      ...
      <div className="px-4 pb-40 overflow-x-hidden lg:scale-[1.0] md:scale-[1.0] scale-[1.0] lg:mt-6 md:-mt-4 -mt-4">
            ...
            {stewardSeeds.map((seed, index) => {
              ...
              return (
                <StewardSeedCard
                  ...
                  index={index}
                  onTendSeedAction={() =>
                    router.push(`/seed/${seed.id}/${seedSlug}`)
                  }
                  onExploreAction={() =>
                    router.push(`/wallet/steward/${seed.id}`)
                  }
                />
              );
            })}
        ...
        {tendedEcosystems.length === 0 ? (
          ...
          >
            <div className="text-gray-400 text-lg -mb-2 tracking-wider">
              <span className="peridia-display-light">Y</span>ou {`haven't`}{" "}
              tended
            </div>
            <div className="text-gray-400 text-lg mb-3 peridia-display-light tracking-wide">
              an Ecosystem yet
            </div>
            <div className="text-gray-400 text-lg -mb-2 peridia-display-light tracking-wider">
              Explore the Garden
            </div>
            <div className="text-gray-400 text-lg peridia-display-light tracking-widest">
              to start nurturing one
            </div>
          </motion.div>
        ) : (
          ...
          <>
            <div className="space-y-4 mb-20 mt-6 z-50 lg:mt-6 md:mt-6">
              {tendedEcosystems.map((snapshot, index) => {
                const codeFromIndex = getCodeForIndex(
                  snapshot.beneficiaryIndex
                );
                ...
                return (
                  <TendedEcosystem
                    ...
                    onReadMoreAction={() =>
                      handleReadMore(snapshot.beneficiaryIndex)
                    }
                    onTendAgainAction={() => handleTendAgain(snapshot.id)}
                    onShare={handleShare}
                    index={index}
      ...
      <ShareModal
        ...
        onCloseAction={handleShareModalClose}
